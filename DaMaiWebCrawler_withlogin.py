...
option.add_argument(r'user-data-dir=C:\Users\wangj\AppData\Local\Google\Chrome\User Data backup')
...
url = "https://detail.damai.cn/item.htm?id=695973461503"
...
if os.path.exists(r"D:\\damai_cookies.txt"):
	# 如果有cookies 直接登录

	with open(r"D:\\damai_cookies.txt", "r", encoding="utf-8") as f:
	...
	ToLoginButtons = browser.find_elements(By.CLASS_NAME, "login-user")
	for button in ToLoginButtons:
		if "登录" not in button.text:
			continue
...
with open(r"D:\\damai_cookies.txt", "w") as f:
	f.write(jsonCookies)

# TODO 等待到时间

# 选中城市
CityListButtons = browser.find_elements(By.CLASS_NAME, "cityitem")
for button in CityListButtons:
	print()
	if "dcity_1" not in button.get_attribute("data-spm"):
		continue
	CityButton = button
	actions = ActionChains(browser)
	actions.move_to_element(CityButton)
	actions.click()
	actions.perform()

'''
# 选中场次
TimeButtons = browser.find_elements(By.CLASS_NAME, "select_right_list_item")
for button in TimeButtons:
	if "二" not in button.text:
		continue
	TimeButton = button
	actions = ActionChains(browser)
	actions.move_to_element(TimeButton)
	actions.click()
	actions.perform()
'''
...
try:
	...
	# 点击添加观演人
	visitorButtons = browser.find_elements(By.CLASS_NAME, "next-checkbox-label")
	for button in visitorButtons:
		if "王军懿" not in button.text:
			continue
		CheckVisitorButton = button
		actions = ActionChains(browser)
		actions.move_to_element(CheckVisitorButton)
		actions.click()
		actions.perform()
